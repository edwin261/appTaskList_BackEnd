...
describe("controller method 'addTaskRecord()'", () => {
    it("Crear un nuevo registro de tarea", () => {
        ...
        const response = request(app).post('/task').send(firstTaskRecord);
        expect(response).toMatchObject(/firstTaskRecord/);
    });
});

describe("controller method 'getTaskRecords()'", () => {
    it("Obtener el listado de tareas creadas", () => {
        const response = request(app).get('/task');
...
describe("controller method 'putTaskRecord()'", () => {
    it("editar un registro del listado de tareas creadas", () => {
        ...
        const response = request(app).put('/task').send(firstTaskRecord);
        expect(response).toMatchObject(/firstTaskRecord/);
    });
});
...
describe("controller method 'deleteTaskRecord()'", () => {
    it("eliminar un registro del listado de tareas creadas", () => {
        const response = request(app).delete('/task/1');
        expect(response).toMatchObject(/Delete/);
    });
});